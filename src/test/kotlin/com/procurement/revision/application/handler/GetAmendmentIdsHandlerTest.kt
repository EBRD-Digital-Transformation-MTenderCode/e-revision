...
internal class GetAmendmentIdsHandlerTest {
    ...
    fun handleSuccess() {
        ...
        val amendment = getTestAmendment().copy(id = id, relatedItem = "5407df26-9305-4f1b-b438-8e16d2ab2201")
        whenever(cassandraAmendmentRepository.findBy(any())).thenReturn(listOf(amendment))
        val node = loadJson("json/dto/getAmendmentIds/get_amendment_id_request_full.json").toNode()

        whenever(cassandraAmendmentRepository.findBy(any())).thenReturn(listOf(amendment))

        val actualApi = getAmendmentIdsHandler.handle(node)

        assertEquals(listOf(id), actualApi.result)
    }
